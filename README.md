...
An exploration of 2 weeks was conducted to evaluate and report the feasibility of leveraging local versions of large language models (LLM's) to develop applications like a software engineering team would do in a business context. Open source gen-ai for LLM's were leveraged to convert natural language into a python codebase. The purpose was to build a simplistic carbon accounting tool to identify the carbon footprint caused by assets registered in a configuration management database. During the exploration, a grossly limited version has been 'developed' as Proof of Feasibility. Building an application was found to be feasible although it does take time and requires innevitable sacrifices on security. Context is currently the biggest bottleneck and requires a spoon feeding approach to be productive. During the coarse of the exploration, confidence was developed that the 'context-bottleneck' will definitely disappear in due time. 
...
- Consideration: level of security standards is at best the security standards known at the time the model was made. Newer encryption technologies will never be included in the model. In reverse, the same counts for vulnerabilities: vulnerabilities found after model creation will cause an immediate threat to whatever applications build leveraging the model. 
